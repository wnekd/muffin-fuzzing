...
/**
 * @dev TickMap stores which ticks are initialized.
 */
library TickMaps {
    struct TickMap {
    ...
    /// @dev convert tick into an unsigned integer
    /// @dev assume tick >= Constants.MIN_TICK
    ...
    /// @dev Find the next initialized tick below the given tick
    /// @dev assume tick >= Constants.MIN_TICK
    //
        ...
        unchecked {
            ...
            if (word == 0) {
                ...
                if (block_ == 0) {
                    ...
                    blockIdx = msb(blockmap);
                    block_ = self.blocks[blockIdx];
                }
                wordIdx = (blockIdx << 8) | msb(block_);
                word = self.words[wordIdx];
            }

            tickBelow = _decompress((wordIdx << 8) | msb(word));
        }
    }
